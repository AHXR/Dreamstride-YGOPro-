...
function c90000017.initial_effect(c)
	...
	e1:SetDescription(aux.Stringid(90000017,0))
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e1:SetType(EFFECT_TYPE_IGNITION)
	e1:SetCountLimit(1)
	e1:SetRange(LOCATION_MZONE)
	e1:SetTarget(c90000017.tg)
	e1:SetOperation(c90000017.op)
	c:RegisterEffect(e1)
end

function c90000017.filter(c,e,tp)
...
function c90000017.op(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,c90000017.filter,tp,LOCATION_DECK,0,1,1,nil,e,tp)
