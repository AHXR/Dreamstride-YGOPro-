...
function c90000029.initial_effect(c)
	...
	e1:SetCountLimit(1)
	e1:SetCost(c90000029.cost)
	e1:SetTarget(c90000029.target)
	e1:SetOperation(c90000029.operation)
	c:RegisterEffect(e1)
end
----------------------------------------------------------------------
function c90000029.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetActivityCount(tp,ACTIVITY_ATTACK)==0 end
	if chk==0 then return e:GetHandler():CheckRemoveOverlayCard(tp,1,REASON_COST) end
	e:GetHandler():RemoveOverlayCard(tp,1,1,REASON_COST)
end
function c90000029.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c90000029.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and c:IsFaceup() and tc:IsRelateToEffect(e) and Duel.Remove(tc,tc:GetPosition(),REASON_EFFECT+REASON_TEMPORARY)~=0 then
		...
		e1:SetCountLimit(1)
		e1:SetCondition(c90000029.retcon)
		e1:SetOperation(c90000029.retop)
		e1:SetLabel(1)
		e1:SetLabelObject(tc)
		Duel.RegisterEffect(e1,tp)
	end
end
function c90000029.retcon(e,tp,eg,ep,ev,re,r,rp)
	return tp==Duel.GetTurnPlayer()
end
function c90000029.retop(e,tp,eg,ep,ev,re,r,rp)
	local t=e:GetLabel()
	if t==2 then e:SetLabel(0)
